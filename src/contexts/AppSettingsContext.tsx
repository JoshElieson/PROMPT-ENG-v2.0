...
function validateSettingValue(
  ...
  | { ok: true; value: AppSettings[keyof AppSettings] }
...
export function AppSettingsProvider({ children }: { children: ReactNode }) {
  ...
  const isValidSettingKey = useCallback(
    (key: string): key is keyof AppSettings => {
  ...
  const applySettingChange = useCallback(
    (request: SettingChangeRequest): ApplySettingResult => {
      if (!isValidSettingKey(request.settingKey)) {
        return {
          ok: false,
          reason: `Unknown setting key: ${request.settingKey}`,
        };
      }
