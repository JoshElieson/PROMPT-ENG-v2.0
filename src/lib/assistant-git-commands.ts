...
export function parseUserGitCommand(text: string): GitAssistantCommand | null {
  ...
  const withoutPrefix = lower.replace(/^git\s+/, "").trim();

  if (/^(status|st)$/i.test(withoutPrefix)) return { action: "status" };
  if (/^(pull|pl)$/i.test(withoutPrefix)) return { action: "pull" };
  if (/^(push|pu)$/i.test(withoutPrefix)) return { action: "push" };
  if (/^(fetch|fe)$/i.test(withoutPrefix)) return { action: "fetch" };
  if (/^(init)$/i.test(withoutPrefix)) return { action: "init" };

  const commitMatch = trimmed.match(
    /^(?:git\s+)?commit(?:\s+(?:-m\s+|--message\s+))?(.+)$/i,
  );
  if (commitMatch?.[1]) {
    const message = commitMatch[1].trim().replace(/^["']|["']$/g, "");
    if (message) {
      const stageAll =
        /\b(all|everything|stage all|commit all)\b/i.test(trimmed) ||
        /\b(-a|--all)\b/i.test(trimmed);
      return { action: "commit", message, stageAll };
    }
  }

  if (/\b(commit all|commit everything|stage all and commit)\b/i.test(trimmed)) {
    const messageMatch = trimmed.match(/(?:message|with)\s+["']([^"']+)["']/i);
    const message = messageMatch?.[1]?.trim() || "update project files";
    return { action: "commit", message, stageAll: true };
  }

  if (/\b(discard|revert|restore)\b/i.test(trimmed)) {
    const pathsMatch = trimmed.match(/(?:files?|paths?)\s+(.+)$/i);
    const paths = pathsMatch?.[1]
      ?.split(/,|\band\b/i)
      .map((part) => part.trim().replace(/^["']|["']$/g, ""))
      .filter(Boolean);
    if (paths?.length) {
      return { action: "restore", paths };
    }
  }
