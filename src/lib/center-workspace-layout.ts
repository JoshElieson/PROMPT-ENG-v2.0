...
function mapLeaf(
  ...
  leafId: string,
  fn: (l: WorkspaceLeafNode) => WorkspaceLeafNode,
): WorkspaceLeafNode {
  return leaf.id === leafId ? fn(leaf) : leaf;
}

export function assignLeafThreadId(
  ...
  switch (root.kind) {
    case "single":
      return {
        ...
        root: {
          kind: "single",
          leaf: mapLeaf(root.leaf, leafId, (l) => ({ ...l, threadId })),
        },
      };
    case "double":
      return {
        ...
        root: {
          ...root,
          first: mapLeaf(root.first, leafId, (l) => ({ ...l, threadId })),
          second: mapLeaf(root.second, leafId, (l) => ({ ...l, threadId })),
        },
      };
    case "triple":
      return {
        ...
        root: {
          ...root,
          topLeft: mapLeaf(root.topLeft, leafId, (l) => ({ ...l, threadId })),
          topRight: mapLeaf(root.topRight, leafId, (l) => ({ ...l, threadId })),
          bottom: mapLeaf(root.bottom, leafId, (l) => ({ ...l, threadId })),
        },
      };
    case "quad":
      return {
...
export function updateDoubleSizes(
  layout: WorkspacePaneLayout,
  sizes: [number, number],
): WorkspacePaneLayout {
  if (layout.root.kind !== "double") return layout;
  return {
    ...layout,
    root: { ...layout.root, sizes: normalizePair(sizes[0], sizes[1]) },
  };
}

export function updateTripleVerticalSizes(
  layout: WorkspacePaneLayout,
  sizes: [number, number],
): WorkspacePaneLayout {
  if (layout.root.kind !== "triple") return layout;
  return {
    ...layout,
    root: {
      ...layout.root,
      verticalSizes: normalizePair(sizes[0], sizes[1]),
    },
  };
}

export function updateTripleTopHorizontalSizes(
  layout: WorkspacePaneLayout,
  sizes: [number, number],
): WorkspacePaneLayout {
  if (layout.root.kind !== "triple") return layout;
  return {
    ...
    root: {
      ...layout.root,
      topHorizontalSizes: normalizePair(sizes[0], sizes[1]),
    },
  };
}

export function updateQuadVerticalSizes(
  layout: WorkspacePaneLayout,
  sizes: [number, number],
): WorkspacePaneLayout {
  if (layout.root.kind !== "quad") return layout;
  return {
    ...layout,
    root: {
      ...layout.root,
...
export function updateQuadTopHorizontalSizes(
  layout: WorkspacePaneLayout,
  sizes: [number, number],
): WorkspacePaneLayout {
  if (layout.root.kind !== "quad") return layout;
...
export function updateQuadBottomHorizontalSizes(
  layout: WorkspacePaneLayout,
  sizes: [number, number],
): WorkspacePaneLayout {
  if (layout.root.kind !== "quad") return layout;
  return {
    ...
    root: {
      ...layout.root,
      bottomHorizontalSizes: normalizePair(sizes[0], sizes[1]),
    },
  };
}
